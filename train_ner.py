...
from data_utils.label_map import DATA_META, GLOBAL_MAP, DATA_TYPE, DATA_SWAP, TASK_TYPE, PREDICT_META, generate_decoder_opt
...
def data_config(parser):
    parser.add_argument('--log_file', default='mt-dnn-ner-train.log', help='path for log file.')
    ...
    parser.add_argument('--name', default='xiaodl job')
    parser.add_argument('--train_datasets', default='ener')
    parser.add_argument('--test_datasets', default='ener')
    parser.add_argument('--pw_tasks', default='qnnli,swag', type=str)
    return parser
...
def main():
    ...
    for dataset in args.train_datasets:
        ...
        task_types_bool.append(task_type > 1)
    task_types_bool = set(task_types_bool)
    if len(task_types_bool) > 1:
        args.mtl_opt = 0
    #### end valid check
    ...
    for dataset in args.train_datasets:
        ...
        if task_id < len(decoder_opts):
            ...
            assert task_types[task_id] == task_type
        else:
    ...
    for dataset in args.test_datasets:
        ...
        if os.path.exists(dev_path):
            dev_data = BatchGen(BatchGen.load(dev_path, False, pairwise=pw_task, maxlen=args.max_seq_len),
                                  ...
                                  data_type=data_type,
                                  task_type=task_type,
                                  reduce_on=PREDICT_META[prefix] > 0)
